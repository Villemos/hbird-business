...
public abstract class BaseLimitChecker {
    ...
    /** The class logger. */
    private static Logger logger = LoggerFactory.getLogger(BaseLimitChecker.class);
    ...
    public State processParameter(@Body Parameter newParameter) {
        logger.info("Limit state '{}' received parameter value for validation.", limit.getName());
    ...
    public State processLimit(@Body Parameter newLimit) {
        ...
        logger.info("Limit '{}' set to '{}'.", limit.getName(), limit.getValue());
        return doProcess(lastValue);
    }

    /**
     * Method for receiving a notification of a enable / disable switch. The method is expected called as part of a
     * camel
    ...
    public State processEnabled(@Body State newEnabled) throws Exception {
        limit.setEnabled(newEnabled.getValue());
        logger.info("Limit '{}' switches to ENABLED state '{}'.", limit.getName(), limit.getEnabled());
        return doProcess(lastValue);
    }
    ...
    protected State doProcess(Parameter parameter) {
        ...
        if (isEnabled() == true && isReady()) {
            ...
            state.setApplicableTo(parameter.getID());
            state.setValue(checkLimit(parameter));
