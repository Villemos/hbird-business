 ...
 * 
 * The API wraps the underlying hbird control mechanisms. In its basic form a request in hbird consists of <li>Creation
 * of a DataRequest object from the org.hbird.exchange.dataaccess module.</li> <li>The publishing of the request to the
 * system as a Request-Reply (in-out) exchange</li> <li>The reception and processing of the request by the archive and
 * the return of the reply</li> <li>The clients reception of the reply</li>
 * 
 ...
 * easy to use format.
 * 
 * The API supports two different 'modes' of operation <li>Initialization. An initialization retrieval is to get the
 * sample(s) of one or more parameters applicable at a given time. For each parameter, X samples are returned which was
 * the value of the parameter at the given time. Typically this is used to initialize a display or component with the
 * latest value of a set of parameters.</li> <li>Stepping. A stepping retrieval is to get a chronologically complete
 * list of samples of a set of parameters from a time to another time. The list may contain samples for each parameter,
 * but might not. This is typically used to step through history of one or more parameters from a starting time to an
 * end time.
 * 
 * In both modes the States of each parameter sample can also be retrieved. This is done using the methods which name
 * ends with 'AndStates'.
 * 
 * Each of the two modes are supported by a set of methods. Each method takes a combination / subset of the following
 * arguments <li>name(s). A Sting, or a List of Strings, with the name(s) of the parameter(s).</li> <li>from. The start
 * time of the sample set.</li> <li>to/at. The end time of the sample set. For initialization requests this is called
 * 'at'.</to> <li>rows. Limits the maximal number of entries to be returned.</li>
 * 
...
public interface IDataAccess extends IHbirdApi {
     ...
     * </li> <li>Starting from the last received sample of the parameter</li> <li>and sorted on timestamp in DECENDING
     * order</li>
     * 
     * @param name The name of the parameter
...
/**
     ...
     * <li>Starting from the 'at' time</li> <li>and sorted on timestamp in DECENDING order</li>
     * 
     * @param name The name of the parameter
     * @param at Java time which the sample value must be less than
     * @param rows The maximum number of entries to be retrieved
...
/**
     ...
     * sample</li> <li>With the highest possible timestamp</li>
     * 
     * @param name The name of the parameter
     ...
     * <li>A maximum of 'rows' entries</li> <li>Starting from the last received sample of the parameter</li> <li>and
     * sorted on timestamp in DECENDING order</li>
     * 
     * @param name The name of the parameter
     * @param rows The maximum number of entries to be retrieved
     * @return Map keyed on the parameter sample and containing as key a list of states applicable to the specific
...
/**
     ...
     * 'rows' entries</li> <li>Starting from the 'at' time</li> <li>and sorted on timestamp in DECENDING order</li>
     * 
     * @param name The name of the parameter
     * @param at Java time which the sample value must be less than
     * @param rows The maximum number of entries to be retrieved
     ...
     * parameter sample</li> <li>With the highest possible timestamp</li>
     * 
     * @param names The names of the parameters
     * @return Map keyed on the parameter sample and containing as key a list of states applicable to the specific
     *         sample.
     ...
     * of 'rows' entries</li> <li>Starting from the last received sample of the parameter</li> <li>and sorted on
     * timestamp in DECENDING order</li>
     * 
     * @param names The names of the parameters
     * @param rows The maximum number of entries to be retrieved
     * @return Map keyed on the parameter sample and containing as key a list of states applicable to the specific
...
/**
     ...
     * <li>With a sample timestamp between 'from' and 'to'</li> <li>and sorted on timestamp in ASCENDING order</li>
     * 
     ...
     * samples</li> <li>With a sample timestamp between 'from' and 'to'</li> <li>and sorted on timestamp in ASCENDING
     * order</li>
     * 
     * Notice that if the 'rows' is below the number of samples between 'from' and 'to'
     * then only a subset of the total samples will be returned. The sampling will start
     ...
     * return <li>All parameter samples and their states</li> <li>With a sample timestamp between 'from' and 'to'</li>
    ...
    /**
     * Method to retrieve the samples of a single parameter and all states applicable to each sample. The method will
     * return <li>A maximum of 'rows' entries</li> <li>With a sample timestamp between 'from' and 'to'</li> <li>and
     * sorted on timestamp in ASCENDING order</li>
     * 
