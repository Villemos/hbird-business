...
public class TleOrbitalPredictor {
    ...
    private TleOrbitalParameters getTleParameters(String satellite, CamelContext context) {
        TleRequest request = new TleRequest(StandardComponents.ORBIT_PREDICTOR, satellite);
    ...
    private List<GroundStation> getLocations(List<String> list, CamelContext context) {
        GroundStationRequest request = new GroundStationRequest(StandardComponents.ORBIT_PREDICTOR, list);
        Exchange arg1 = new DefaultExchange(context);
        arg1.getIn().setBody(request);
        context.createProducerTemplate().send("direct:locationstore", arg1);
        List<GroundStation> stations = (List<GroundStation>) arg1.getOut().getBody();
        return stations;
    }
