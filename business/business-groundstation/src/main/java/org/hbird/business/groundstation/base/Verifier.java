...
public class Verifier {
    ...
    public synchronized State verify(@Body String response, @Header(SetHamlibNativeCommandHeaders.HEADER_STAGE) String stage,
        ...
        if (verificationStages.containsKey(derivedFrom)) {
            ...
            if (response.contains("RPRT 0") == false) {
                ...
                LOG.debug("Native command '" + commandid + "' succeeded; '" + response.replaceAll("\n", "") + ". Removing commandid '" + commandid
                        + "' from stage '" + stage + "' of command '" + derivedFrom + "'.");
    ...
    protected State createState(String stage, boolean state, String derivedFrom) {
        ...
        State stateObj = new State("/Rotator/" + stage, stage);
        stateObj.setDescription(stageDescription.get(stage));
        stateObj.setApplicableTo(derivedFrom);
        stateObj.setValue(state);
        return stateObj;
    }
}
